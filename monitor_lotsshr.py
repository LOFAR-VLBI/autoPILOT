...
number_workers = sum(
    [THREAD_POOL_SIZES[x] for x in THREAD_POOL_SIZES.keys()]
)
...
with ProcessPoolExecutor(max_workers=number_workers) as executor:
    ...
    while True:
        ...
        current_status = get_thread_status(futures)
        print_thread_status(current_status)
    
        if (
            current_status["stage"]["running"] <= THREAD_POOL_SIZES["stage"]
            and nextfield is not None
        ):
        ...
        if (
            ksum < totallimit
            and 'Staged' in d
            and len(current_status["download"]["running"]) < THREAD_POOL_SIZES["download"]
        ):
            download_name=fd['Staged'][0]
            print('We need to download a new file (%s)!' % download_name)
            futures["download"].append(executor.submit(do_download, download_name))
    
        ## unpack the files if downloaded
        if (
            'Downloaded' in d
            and len(current_status["unpack"]["running"]) <= THREAD_POOL_SIZES["unpack"]
        ):
            unpack_name=fd['Downloaded'][0]
            print('We need to unpack a new file (%s)!' % unpack_name)
            futures["unpack"].apend(executor.submit(do_unpack, unpack_name))
            ...
            for field in fd['Unpacked']:
                if nq > THREAD_POOL_SIZES["queue"]:
                    ...
                    for obsid in field_obsids:
                        ...
                        futures["queue"].append(executor.submit(run_task, fieldobsid, next_task))
                        ## NEED TO FIX
        ...
        if 'Queued' in d:
            for field in fd['Queued']:
                ...
                if result == 'Running':
                    ...
                    if (
                        len(remaining_tasks) > 0
                        and len(current_status["queue"]["running"]) < THREAD_POOL_SIZES["queue"]
                    ):
        ...
        if (
            'Verified' in d
            and len(current_status["verify"]["running"]) < THREAD_POOL_SIZES["verify"]
        ):
            verify_name = fd['Verified'][0]
            futures["verify"]["running"].append(executor.submit(do_verify, verify_name))
